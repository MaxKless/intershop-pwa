...
const DEPLOY_URL_WITH_SLASH = getDeployURLFromEnv();
  ...
  server.engine(
    ...
    ngExpressEngine({
      ...
      providers: [
        ...
        { provide: DEPLOY_URL_SERVER, useValue: DEPLOY_URL_WITH_SLASH },
      ],
    })
  );
  ...
  server.get(/\/.*\.(js|css)$/, (req, res) => {
    ...
    fs.readFile(join(DIST_FOLDER, 'browser', path), { encoding: 'utf-8' }, (err, data) => {
      if (err) {
        ...
        res.send(setDeployUrlInFile(DEPLOY_URL_WITH_SLASH, path, data));
      }
    });
  });
  ...
  server.get(
    ...
    express.static(join(DIST_FOLDER, 'browser'), {
      setHeaders: (res, path) => {
        ...
        if (
          DEPLOY_URL_WITH_SLASH.startsWith('http') &&
    ...
    res.render(
      ...
      (err, html) => {
        if (html) {
          ...
          newHtml = setDeployUrlInFile(DEPLOY_URL_WITH_SLASH, req.originalUrl, newHtml);
